...
while opcion != "0":
  if opcion == "1":
    ...
    encontrado = False
    consultaEnlace = consultarEnlace(paisABuscar.upper())

    if len(consultaEnlace) != 0:
      encontrado = True

      for enlace in consultaEnlace:
        enlaceABuscar = "https://datosmacro.expansion.com" + (enlace[0])
    ...
    if opcionContinente == 0:
      ...
      tablaPorContinente = consultarPorContinente(continenteElegido)

      for pais in tablaPorContinente:
          listaNombrePais.append(pais[0]) #NOMBRE
          listaIndicePais.append(float(pais[1])) #INDICE
